            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    <div class="header">
                        ...
                        <a href="{{ url('system/files/create') }}" class="btn bg-red waves-effect pull-right">Create</a>
                    </ul>
                </div>
                <div class="body">
                        ...
                        <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                            ...
                            <tbody id="result">
                                ...
                                <tr>
                                    <td>{{ $file->id }}</td>
                                    <td>
                                        <?php if ($file->enable == 0){ ?>
                                            <div class="label label-danger">No</div>
                                        <?php }else if ($file->enable == 1){ ?>
                                            <div class="label label-success">Yes</div>
                                        <?php } ?>
                                    </td>
                                    <td>{{ $file->title }}</td>
                                    <td>{{ $file->lesson_title }}</td>
                                    <td>{{ $file->source }}</td>
                                    <td>{{ $file->created_at }}</td>
                                    <td>{{ $file->updated_at }}</td>
                                    <td>
                                        <form id="{{ $file->id }}" action="{{ url('system/files/'.$file->id) }}" method="post">
                                            {{ csrf_field() }}
                                            <input type="hidden" name="_method" value="DELETE">
                                            <div class="btn-group" role="group" aria-label="Default button group">
                                                <a href="{{ url('system/files/'.$file->id) }}/edit" class="btn bg-pink waves-effect"><i class="material-icons">mode_edit</i></a>
                                                <button type="button" class="btn bg-pink waves-effect" onclick="if (confirm('Are you sure?')) { $('#{{$file->id}}').submit() }"><i class="material-icons">delete</i></button>
                                            </div>
                                        </form>
                                    </td>
                                </tr>
