            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    <div class="header">
                        ...
                        <a href="{{ url('system/videos/create') }}" class="btn bg-red waves-effect pull-right">Create</a>
                    </ul>
                </div>
                <div class="body">
                        ...
                        <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                            ...
                            <tbody id="result">
                                ...
                                <tr>
                                    <td>{{ $video->id }}</td>
                                    <td>
                                        <?php if ($video->enable == 0){ ?>
                                            <div class="label label-danger">No</div>
                                        <?php }else if ($video->enable == 1){ ?>
                                            <div class="label label-success">Yes</div>
                                        <?php } ?>
                                    </td>
                                    <td>{{ $video->title }}</td>
                                    <td>{{ $video->lessons_title }}</td>
                                    <td>{{ $video->created_at }}</td>
                                    <td>{{ $video->updated_at }}</td>
                                    <td>
                                        <form id="{{ $video->id }}" action="{{ url('system/videos/'.$video->id) }}" method="post">
                                            {{ csrf_field() }}
                                            <input type="hidden" name="_method" value="DELETE">
                                            <div class="btn-group" role="group" aria-label="Default button group">
                                                <a href="{{ url('system/videos/'.$video->id) }}/edit" class="btn bg-pink waves-effect"><i class="material-icons">mode_edit</i></a>
                                                <button type="button" class="btn bg-pink waves-effect" onclick="if (confirm('Are you sure?')) { $('#{{$video->id}}').submit() }"><i class="material-icons">delete</i></button>
                                            </div>
                                        </form>
                                    </td>
                                </tr>
